...
def index():
    ...
    top_organization = get_top_organization()
    organizations = get_organizations()
    return render_template('index.html', params=[[bar, mapp, scatter], stats, years, locations, title, top_organization, city_l, country_l, organizations])
@app.route('/organize')
def organize():
    stats = predictStats()
    print(stats)
    mapp = create_organize_map()
    scatter = create_organize_scatter()
    years = get_years()
    locations = get_locations()
    title = title_formatting()
    top_organization = get_top_organization()
    organizations = get_organizations()
    return render_template('organize.html', params=[[None,mapp, scatter], stats, years, locations, title, top_organization, city_l, country_l])
...
def create_scatter_plot():
    ...
    with sqlite3.connect('ocean_plastic.db') as con:
        try:
            ...
            if params["org"]:
                plastic_all_data = pd.read_sql(query + 'Where Organization = \'' + params["org"]+"\'" + condition, con)
            ...
            if params["start_year"] and params["end_year"]:
                ...
                plastic_all_data = plastic_all_data[plastic_all_data["DateOriginal"].isin(pd.date_range('1/1/'+startyear,'30/12/'+endyear))]
            ...
            if not params["month"] and not params["year"]:
                ...
                fig = px.line(plastic_all_data, x=plastic_all_data.index, y=plastic_all_data['TotalItemsRecorded'],width=width, height=height,
                              labels={
...
def get_info_box():
    ...
    with sqlite3.connect('ocean_plastic.db') as con:
        try:
            ...
            return json.dumps([organization, description, event_type, total_length, total_items_events_recorded, beachlocation, x, y])
        except pd.io.sql.DatabaseError as e:
            print(e)
            return json.dumps([organization, description, event_type, total_length, total_items_events_recorded, beachlocation, x, y])
        return json.dumps([organization, description, event_type, total_length, total_items_events_recorded, beachlocation, x, y])
    ...
    with sqlite3.connect('ocean_plastic.db') as con:
        try:
            ...
            if country_code and len(name_top_10) == 0:
                name_top_10 = pd.read_sql(query + ' WHERE [ISO_2DIGIT]=' + country_code + ' OR [ISO_3DIGIT] ='+country_code + condition , con)
            elif params["org"]:
...
def create_map():
    ...
    if params["month"]:
        datelimit = params["month"].replace("'", "") +"/%/"+params["year"].replace("'", "")
        condition = condition + ' where DateOriginal like \'' + datelimit + '\''
        if params["org"]:
            condition += ' and Organization = \'' + params["org"]+"\'"
    else:
        if params["org"]:
            condition += ' where Organization = \'' + params["org"]+"\'"
    ...
    if params["mapby"]:
        fig = px.scatter_mapbox(all_data, lat="Latitude1", lon="Longitude1", hover_name="Location",
                                hover_data=["NAME", "COUNTRY", "ISO_CC", "TotalVolunteers", "EventType", "Location"], color='Year', size="Totalltems_EventRecord",
                                color_discrete_sequence=px.colors.qualitative.Bold,
                                zoom=1, width=667, height=334, category_orders={"Year": ["2015", "2016", "2017", "2018", "2019", "2020"]})
    else:
        fig = px.scatter_mapbox(all_data, lat="Latitude1", lon="Longitude1", hover_name="Location",
                                hover_data=["NAME", "COUNTRY", "ISO_CC", "TotalVolunteers", "EventType", "Location"], color='Year',
                                color_discrete_sequence=px.colors.qualitative.Bold,
                                zoom=1, width=667, height=334, category_orders={"Year": ["2015", "2016", "2017", "2018", "2019", "2020"]})
    fig.update_traces(
        hovertemplate=None
    ...
    with sqlite3.connect('ocean_plastic.db') as con:
        try:
            ...
            fig = px.scatter_mapbox(nearest_locations, lat="Y", lon="X", hover_name="Location", color='Year',
                                    color_discrete_sequence=px.colors.qualitative.Bold,
                                    zoom=10, width=667, height=334, category_orders={"Year": ["2015", "2016", "2017", "2018", "2019", "2020"]})
            fig.update_traces(
                hovertemplate=None
            )
            fig.update_layout(mapbox_style="mapbox://styles/falsaadeh/ckf7f7mx70d6d19qkal4k4u99",
